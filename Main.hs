...
gather m = catMaybes $ map map_def (moduleDefinitions m)
  where
    ...
    map_terminator block_terminator = case block_terminator of
      ...
      CondBr condition trueDest falseDest m -> ("cond", Just condition, [trueDest, falseDest], ["t", "f"])
...
t spine_length font spine = renderDrawing size size black $
...
main = do
  ...
  let ex = renderDrawing 400 200 white $
